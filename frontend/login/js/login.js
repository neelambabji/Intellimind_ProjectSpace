...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tabConfig = {
    student:  { placeholder: "e.g. 23A91A0509",  hint: "Use your institution-assigned Roll Number", label: "Roll Number" },
    employee: { placeholder: "e.g. EMP20240012", hint: "Use your Employee ID from HR portal",       label: "Employee ID" },
    admin:    { placeholder: "e.g. ADM001",      hint: "Use your Admin credentials",               label: "Admin ID"    },
  };
  ...
  window.setTab = function (btn, type) {
    ...
    const hintText = document.querySelector(".input-hint-text");
    if (hintText) hintText.textContent = cfg.hint;
    const idLabel = document.getElementById("idLabel");
    if (idLabel) idLabel.firstChild.textContent = cfg.label + " ";
    clearErrors();
    userIdInput.value = "";
  };
